...
**Usage rule (echoed in `wiki/CLAUDE.md`):** treat deepwiki answers as **hypotheses to verify** against the upstream Geant4 source on GitHub ([v11.4.0](https://github.com/Geant4/geant4/tree/v11.4.0), the pinned container version) before citing them in a wiki synthesis page. Citation discipline is weaker than reading the source — in our smoke test the tool gave the correct optical-photon PDG = −22 but did not name `G4OpticalPhoton.cc:67` even when asked. If a deepwiki claim checks out against the GitHub source, it earns a place in a synthesis page — citing the `.cc` file (GitHub permalink), not deepwiki.
...
**Geant4 source via GitHub (no local clone).** The wiki's `sources/geant4-code/synthesis/` pages cite `.cc:line` ranges against the upstream Geant4 repo at the pinned container version ([github.com/Geant4/geant4/tree/v11.4.0](https://github.com/Geant4/geant4/tree/v11.4.0)) — browse or permalink lines directly on GitHub. Nothing is downloaded or shipped, and `geant4-init` has no source-checkout step. The cited version tracks `bin/g4run`'s pinned image tag (single source of truth), so a container bump moves the citation target with it. (Earlier versions offered an optional local `geant4-src` tarball + a `wiki/raw/geant4-src` symlink; removed 2026-05-31 — GitHub links are zero-setup and always match the tag.)
