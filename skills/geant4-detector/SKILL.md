---
name: geant4-detector
description: Use when the user wants to turn a natural-language detector description (shapes, materials, sizes, sensitive volumes, optical radiators) into a validated standalone GDML file under geometries/. Validates in-container before declaring success. Requires geant4-init to have run.
---

# geant4-detector — natural-language spec to validated GDML
...
6. **Show the user** the path to the GDML and a one-line summary of what
   was written (shapes, materials, sensitive volumes). Suggest the next
   step, depending on how their `main.cc` consumes geometry:
   - example main: rebuild if needed, then run with the **geant4-run** skill,
     passing the GDML as the first positional arg —
     `--exe build/geant4_claude_main -- geometries/<name>.gdml macros/<name>.mac {run_dir}/hits.root`;
...
- This skill does not run a simulation. It only writes and validates GDML.
